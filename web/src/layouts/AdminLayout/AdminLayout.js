...
const AdminLayout = ({ children }) => {
  ...
  const onSearchSubmit = (data) => {
    if (!searchType) {
      navigate(routes.search({ type: 'contacts', query: data.search }))
    } else {
      navigate(routes.search({ type: searchType, query: data.search }))
    }
  }
                  ...
                  {searchTitle && (
                    ...
                    >
                      <div className="relative w-full text-gray-400 focus-within:text-gray-600">
                        ...
                        <Label name="search" className="sr-only">{`Search${
                          searchTitle ? ' ' + searchTitle : ''
                        }`}</Label>
                        <TextField
                          id="search_field"
                          className="block max-w-full w-full h-full pl-8 pr-3 py-2 border-transparent text-gray-900 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-0 focus:border-transparent sm:text-sm"
                          placeholder={`Search${
                            searchTitle ? ' ' + searchTitle : ''
                          }`}
                          defaultValue={query}
                          ...
                          name="search"
                        />
                        <Submit className="sr-only">Submit</Submit>
                      </div>
                    </Form>
                  )}
                </div>
